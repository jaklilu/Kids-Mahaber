...
export function ensureScheduleProposal(data: TrackerData): TrackerData {
  ...
  // Never silently revert Frea from Aug 15 back to second-Saturday Aug 8.
  const frea = updated.members.find((m) => m.name === "Frea");
  if (frea?.proposedDate === "2026-08-08") {
    updated = {
      ...updated,
      members: updated.members.map((m) =>
        m.name === "Frea"
          ? { ...m, proposedDate: FREA_FIRST_HOST_DATE }
          : m,
      ),
    };
  }
